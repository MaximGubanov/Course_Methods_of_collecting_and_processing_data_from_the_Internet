...
def parse_page(vacancies):
    ...
    return (vacancies_list_page, len(vacancies_list_page))
...
def get_content_page(search_param, page_next=True):
    ...
    while page_next is not None:
        ...
        parsed_page, len_pages = parse_page(vacancies)
        pages += parsed_page
        params['page'] += 1

    return (pages, params['page'], len(pages))
...
if __name__ == '__main__':
    ...
    with open('result_parse.json', '+a', encoding='utf-8') as file:
        json.dump(all_vacancies, file, indent=2)